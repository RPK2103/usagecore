...
/**
 * Workload B: durable {@code POST /api/v1/usage/events}. Measures HTTP 202 acceptance only.
 * Kafka/ledger completion is a separate drain measurement.
 */
public class UsageEventsSimulation extends Simulation {

    {
        ...
        ScenarioBuilder scn = scenario("usage-events-" + profile.name().toLowerCase())
                .exec(http("POST /api/v1/usage/events")
                        .post("/api/v1/usage/events")
                        .header("Content-Type", "application/json")
                        .header("X-Correlation-Id", session -> "perf-events-" + runId + "-" + session.userId())
                        .body(StringBody(session -> {
                            String key = "perf-" + runId + "-" + UUID.randomUUID();
                            return """
