# Local observability (Phase 9A)

Applications normally run on the host (IDE / Maven). Prometheus and the OpenTelemetry Collector run in Docker.
...
Grafana is not part of Phase 9A dashboard work. Do not treat listed Prometheus targets as healthy unless the three applications are actually running.
...
| `USAGECORE_TRACING_SAMPLING_PROBABILITY` | `1.0` | Trace sampling |

If the collector is down, business processing continues. Tracing is best-effort.

## Readiness reminder (Usage Pipeline)

HTTP `POST /api/v1/usage/events` is durable after PostgreSQL commit of ingestion + outbox. Kafka unavailability must not mark the process unready for HTTP acceptance. Watch `usagecore_outbox_pending` instead.
